...
type BeibuwanCasAuthHandler struct {
	once                sync.Once
...
func (l *BeibuwanCasAuthHandler) GetTicket(c *gin.Context) (ticket string, err error) {
	l.once.Do(func() {
...
func (l *BeibuwanCasAuthHandler) CheckTicket(ticket string) (ticketBody []byte, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("post %s 失败：%s", httpReq.RequestURI, err.Error())
	}
	if httpResp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("post %s HttpStatus异常：%d", httpReq.RequestURI, httpResp.StatusCode)
	}
	defer httpResp.Body.Close()
	return ioutil.ReadAll(httpResp.Body)
}

// func (l *BeibuwanCasAuthHandler) GetUserUid(ticketBody []byte) (uid string, err error) {
...
func (l *BeibuwanCasAuthHandler) GetUserUid(ticketBody []byte) (uid string, err error) {
	str := (*string)(unsafe.Pointer(&ticketBody))
	path := xmlpath.MustCompile(l.casAuthTicketPath)
	root, err := xmlpath.Parse(strings.NewReader(*str))
	if err != nil {
		return "", err
	}
	if value, ok := path.String(root); ok {
		fmt.Println("Found User Uid:", value)
		return value, nil
	}
	return "", fmt.Errorf("解析用户信息: %s失败，body:%s", l.casAuthTicketPath, ticketBody)
}

func (l *BeibuwanCasAuthHandler) CreateToken(uid string, c *gin.Context) (user *model.User, expiry int64, tokenStr string, err error) {
